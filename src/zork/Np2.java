...
public class Np2
{
	...
	int schlst_(int oidx, int aidx, int rm, int cn, int ad, int spcobj)
	{
		...
		{
			/* SEARCH OBJECTS. */
			if ((vars.objcts_1.oflag1[i - 1] & Vars.VISIBT) == 0 || ((rm == 0 || !game.dsub.qhere_(i, rm))
					&& (cn == 0 || vars.objcts_1.ocan[i - 1] != cn) && (ad == 0 || vars.objcts_1.oadv[i - 1] != ad)))
			{
//	    		goto L1000;
				continue;
			}
...
//	L200:
			if ((vars.objcts_1.oflag1[i - 1] & Vars.TRANBT) == 0 && (vars.objcts_1.oflag2[i - 1] & Vars.OPENBT) == 0)
			{
//				goto L1000;
				continue;
			}
			...
			{
				/* SEARCH OBJECTS. */
				if ((vars.objcts_1.oflag1[j - 1] & Vars.VISIBT) == 0 || !thisit_(oidx, aidx, j, spcobj))
				{
//					goto L500;
					continue inner_for;
				}

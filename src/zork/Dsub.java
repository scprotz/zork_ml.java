...
public class Dsub implements Actions
{
	...
	private void rspsb2nl_(int n, int y, int z, int nl)
	{
		int res;
...
//	#endif
	...
	void jigsup_(int desc)
	{
		...
		vars.objcts_1.oflag1[vars.oindex_1.robot - 1] = (vars.objcts_1.oflag1[vars.oindex_1.robot - 1] | Vars.VISIBT)
				& ~Vars.NDSCBT;
		...
		do
		{
			...
			case 300:
				...
				if (full == 0 && (vars.findex_1.superf
						|| (vars.rooms_1.rflag[vars.play_1.here - 1] & Vars.RSEEN) != 0 && vars.findex_1.brieff))
				{
					GOTO = 400;
					continue;
				}
			...
			case 500:
				...
				{
					rspsub_(431, vars.objcts_1.odesc2[vars.advs_1.avehic[vars.play_1.winner - 1] - 1]);
				}
